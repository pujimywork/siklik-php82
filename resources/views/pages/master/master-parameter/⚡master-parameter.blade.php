...
new class extends Component {
    ...
    {
        $this->dispatch('master.parameter.openCreate');
    }

    public function openEdit(int $parId): void
    {
        $this->dispatch('master.parameter.openEdit', parId: $parId);
    }

    public function requestDelete(int $parId): void
    {
        $this->dispatch('master.parameter.requestDelete', parId: $parId);
    }

    #[On('master.parameter.saved')]
    ...
    {
        $q = DB::table('rsmst_parameters')
            ->select('par_id', 'par_desc', 'par_value')
            ->orderBy('par_id');

        if (trim($this->searchKeyword) !== '') {
            $kw = mb_strtoupper(trim($this->searchKeyword));
            $q->whereRaw('UPPER(par_desc) LIKE ?', ["%{$kw}%"]);
        }
    ...
    <header class="bg-white shadow dark:bg-gray-800">
        <div class="w-full px-4 py-2 sm:px-6 lg:px-8">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">
                Parameter sistem untuk konfigurasi internal aplikasi
            </p>
        </div>
    </header>
            ...
            <div class="sticky z-30 px-4 py-3 bg-white border-b border-gray-200 top-20 dark:bg-gray-900 dark:border-gray-700">
                <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                    ...
                    <div class="w-full lg:max-w-md">
                        <x-input-label for="searchKeyword" value="Cari Parameter" class="sr-only" />
                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="text-gray-700 divide-y divide-gray-200 dark:divide-gray-700 dark:text-gray-200">
                            @forelse ($this->rows as $row)
                                <tr wire:key="parameter-{{ $row->par_id }}"
                                    ...
                                    <td class="px-4 py-3 text-right font-mono">
                                        {{ number_format((float) $row->par_value, 0, ',', '.') }}
                                    </td>
