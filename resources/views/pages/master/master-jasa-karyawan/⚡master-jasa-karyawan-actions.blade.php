...
new class extends Component {
    ...
    #[On('master.jasa-karyawan.openCreate')]
    public function openCreate(): void
    {
        ...
        $this->dispatch('open-modal', name: 'master-jasa-karyawan-actions');
        $this->dispatch('focus-acte-id');
    }
    ...
    public function openEdit(string $acteId): void
    {
        ...
        $this->resetForm();
        $this->formMode   = 'edit';
        $this->originalId = $acteId;
        $this->form = [
            'acte_id'     => (string) $row->acte_id,
            'acte_desc'   => (string) ($row->acte_desc ?? ''),
            'acte_price'  => (string) ($row->acte_price ?? '0'),
    ...
    public function save(): void
    {
        ...
        $messages = [
            'form.acte_id.required' => 'ID Jasa wajib diisi.',
            'form.acte_id.max'      => 'ID Jasa maksimal 10 karakter.',
            'form.acte_id.regex'    => 'ID Jasa hanya huruf besar / angka / "_" / "-".',
            'form.acte_id.unique'   => 'ID Jasa sudah digunakan.',
            'form.acte_desc.required' => 'Deskripsi wajib diisi.',
            'form.acte_desc.max'      => 'Deskripsi maksimal 100 karakter.',
            'form.acte_price.required' => 'Tarif wajib diisi.',
            'form.acte_price.numeric'  => 'Tarif harus berupa angka.',
            'form.acte_price.min'      => 'Tarif minimal 0.',
            'form.active_status.required' => 'Status wajib dipilih.',
            'form.active_status.in'       => 'Status tidak valid.',
        ];

        $attributes = [
            'form.acte_id'     => 'ID Jasa',
            'form.acte_desc'   => 'Deskripsi',
            'form.acte_price'  => 'Tarif',
        ...
        $payload = [
            'acte_desc'   => mb_strtoupper($this->form['acte_desc']),
            'acte_price'  => (float) $this->form['acte_price'],
    ...
    {
        $this->resetForm();
        $this->dispatch('close-modal', name: 'master-jasa-karyawan-actions');
        $this->resetVersion();
    }

    private function resetForm(): void
    {
        $this->form = ['acte_id' => '', 'acte_desc' => '', 'acte_price' => '0', 'active_status' => '1'];
...
<div>
    <x-modal name="master-jasa-karyawan-actions" size="full" height="full" focusable>
        <div class="flex flex-col min-h-[calc(100vh-8rem)]"
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100">
                                    {{ $formMode === 'edit' ? 'Ubah Jasa Karyawan' : 'Tambah Jasa Karyawan' }}
                                </h2>
                                <p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
                                    Item jasa karyawan (ACTE) untuk billing rawat jalan.
                                </p>
                            </div>
                        </div>
                        <div class="mt-3">
            ...
            <div class="flex-1 px-4 py-4 bg-gray-50/70 dark:bg-gray-950/20"
                 x-data
                 x-on:focus-acte-id.window="$nextTick(() => setTimeout(() => $refs.inputActeId?.focus(), 150))"
                 x-on:focus-acte-desc.window="$nextTick(() => setTimeout(() => $refs.inputActeDesc?.focus(), 150))">

                <x-border-form title="Data Jasa Karyawan">
                    <div class="space-y-4">
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                            <div>
                                <x-input-label value="ID Jasa" />
                                <x-text-input wire:model.live="form.acte_id" x-ref="inputActeId"
                                    maxlength="10"
                                    :disabled="$formMode === 'edit'"
                                    :error="$errors->has('form.acte_id')"
                                    class="w-full mt-1 uppercase"
                                    x-on:keydown.enter.prevent="$refs.inputActeDesc?.focus()" />
                                <x-input-error :messages="$errors->get('form.acte_id')" class="mt-1" />
                            </div>
                            <div class="sm:col-span-2">
                                <x-input-label value="Deskripsi" />
                                <x-text-input wire:model.live="form.acte_desc" x-ref="inputActeDesc"
                                    maxlength="50"
                                    :error="$errors->has('form.acte_desc')"
                                    class="w-full mt-1 uppercase"
                                    x-on:keydown.enter.prevent="$refs.inputActePrice?.focus()" />
                                <x-input-error :messages="$errors->get('form.acte_desc')" class="mt-1" />
                            </div>
                        </div>
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                            <div>
                                <x-input-label value="Tarif (Rp)" />
                                <x-text-input wire:model.live="form.acte_price" x-ref="inputActePrice"
                                    type="number" min="0" step="100"
                                    :error="$errors->has('form.acte_price')"
                                    class="w-full mt-1"
                                    x-on:keydown.enter.prevent="$wire.save()" />
                                <p class="mt-1 text-[11px] text-gray-500 dark:text-gray-400">
                                    Format: angka tanpa titik/koma. Contoh: 50000
                                </p>
                                <x-input-error :messages="$errors->get('form.acte_price')" class="mt-1" />
                            </div>
                            <div>
