...
new class extends Component {
    ...
    {
        $this->dispatch('master.pendidikan.openCreate');
    }

    public function openEdit(int $eduId): void
    {
        $this->dispatch('master.pendidikan.openEdit', eduId: $eduId);
    }

    public function requestDelete(int $eduId): void
    {
        $this->dispatch('master.pendidikan.requestDelete', eduId: $eduId);
    }

    #[On('master.pendidikan.saved')]
    ...
    {
        $q = DB::table('rsmst_educations')
            ->select('edu_id', 'edu_desc')
            ->orderBy('edu_id');

        if (trim($this->searchKeyword) !== '') {
            $kw = mb_strtoupper(trim($this->searchKeyword));
            $q->whereRaw('UPPER(edu_desc) LIKE ?', ["%{$kw}%"]);
        }
            ...
            <div class="sticky z-30 px-4 py-3 bg-white border-b border-gray-200 top-20 dark:bg-gray-900 dark:border-gray-700">
                <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                    ...
                    <div class="w-full lg:max-w-md">
                        <x-input-label for="searchKeyword" value="Cari Pendidikan" class="sr-only" />
                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="text-gray-700 divide-y divide-gray-200 dark:divide-gray-700 dark:text-gray-200">
                            @forelse ($this->rows as $row)
                                <tr wire:key="pendidikan-{{ $row->edu_id }}"
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex flex-wrap gap-2">
                                            <x-secondary-button type="button"
                                                wire:click="openEdit({{ $row->edu_id }})" class="px-2 py-1 text-xs">
                                                Edit
                                            </x-secondary-button>
                                            <x-confirm-button variant="danger"
                                                :action="'requestDelete(' . $row->edu_id . ')'"
                                                title="Hapus Pendidikan"
                                                message="Yakin hapus pendidikan {{ $row->edu_desc }}?"
