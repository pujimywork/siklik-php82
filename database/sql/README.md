...
| `TKTXN_SOWHS` + view `TKVIEW_IOSTOCKWHS` | Stock opname warehouse + re-create view dgn UNION ALL block SO | ⚠️ table di-DROP & re-CREATE — kalau sudah ada data opname **akan hilang**. Lihat catatan teknis di bawah |
...
- **TKTXN_SOWHS data preservation**: Bundle section ini DROP & re-CREATE table. Kalau klinik sudah ada history opname dan mau update view-nya saja (tambah block SO), **jangan** re-run bundle utuh — jalanin manual `CREATE OR REPLACE FORCE VIEW TKVIEW_IOSTOCKWHS AS (...)` saja, dengan UNION ALL block: RCV / SLS / RJ / SO. Grants ke DITOKOKU otomatis ter-preserve.
